...
const categoriasDestacadas = [
  { id: 1, title: "CÁNCER DE MAMA", slug: "cancer-mama", color: "#005c89", icon: iconMamaStatic, iconAnimated: iconMama },
  { id: 2, title: "TU CORAZÓN", slug: "tu-corazon", color: "#005c89", icon: iconCorazonStatic, iconAnimated: iconCorazon },
  { id: 3, title: "DONACIÓN", slug: "donacion", color: "#005c89", icon: iconDonacionStatic, iconAnimated: iconDonacion },
  { id: 4, title: "DENGUE", slug: "dengue", color: "#005c89", icon: iconDengueStatic, iconAnimated: iconDengue },
];
...
const cleanHtmlText = (html) => {
  ...
  return text.replace(/<\/p>|<\/div>|<br\s*\/?>/gi, "\n").replace(/<[^>]*>?/gm, "").replace(/&nbsp;/g, " ").replace(/[ \t]+/g, " ").replace(/\n\s*\n/g, "\n").trim();
};
  ...
  return (
    <article className="full-news-card">
      ...
      <div className="full-news-body">
        ...
        <button className="read-more-btn" onClick={() => onOpenNews(news)}>Ver más</button>
      </div>
      <div className="full-news-images">
        {news.images && news.images.length > 0 && (
          <img src={news.images[0]} alt={news.title} className="clickable-img" onClick={() => onOpenLightbox(news.images, 0)} />
        )}
      </div>
    </article>
  );
...
const NoticiasPage = () => {
  ...
  // Guardará la fecha exacta (timestamp) de la noticia más nueva de cada categoría
  const [nuevasPorCategoria, setNuevasPorCategoria] = useState({});
  ...
  const nextAltaSlide = () => setAltaIndex((prev) => (prev + 1) % miniCarouselData.length);
  const prevAltaSlide = () => setAltaIndex((prev) => (prev - 1 + miniCarouselData.length) % miniCarouselData.length);
  ...
  useEffect(() => {
    const fetchNoticiasPublicas = async () => {
      ...
      try {
        ...
        // 2. DETECCIÓN DE NOTICIAS NUEVAS (Guardamos la fecha de la más reciente)
        const categoriasNuevas = {};
        const unDiaMs = 24 * 60 * 60 * 1000;
        const ahora = new Date().getTime();

        data.forEach(noticia => {
          const catName = (noticia.category || "").trim().toLowerCase();
          
          if (catName !== "general" && catName !== "noticias") {
            const fechaCreacion = noticia.createdAt ? new Date(noticia.createdAt).getTime() : ahora;
            
            if (ahora - fechaCreacion <= unDiaMs) {
               // Guardamos el timestamp de la noticia más reciente de esta categoría
               if (!categoriasNuevas[catName] || fechaCreacion > categoriasNuevas[catName]) {
                 categoriasNuevas[catName] = fechaCreacion;
               }
            }
          }
        });
        
        setNuevasPorCategoria(categoriasNuevas);
  ...
  const nextLightboxImage = (e) => { e.stopPropagation(); setLightbox((prev) => ({ ...prev, index: (prev.index + 1) % prev.images.length })); };
  const prevLightboxImage = (e) => { e.stopPropagation(); setLightbox((prev) => ({ ...prev, index: (prev.index - 1 + prev.images.length) % prev.images.length })); };

  return (
    <main className="noticias-page">
      <div className="news-header-fluid" style={{ backgroundImage: `linear-gradient(rgba(46, 111, 196, 0.85), rgba(233, 235, 238, 0.85)), url(${fondoBannerNoticias})` }}>
      ...
      <div className="noticias-page-container">
        {loading ? (
          ...
          <div className="slider-wrapper">
            <button className="slider-arrow left-arrow" onClick={() => scrollNews("left")}>&#10094;</button>
            <div className="news-slider" ref={sliderRef}>
              {sortedNews.map((news) => (
                <NewsCard key={news.id} news={news} onOpenNews={setSelectedNews} onOpenLightbox={openLightbox} />
              ))}
            </div>
            <button className="slider-arrow right-arrow" onClick={() => scrollNews("right")}>&#10095;</button>
          </div>
        ) : (
        ...
        <section id="articulos-medicos" className="medical-articles-section">
          <div className="medical-left-column">
            ...
            <div className="mini-carousel-card">
              <div className="mini-carousel-img-wrapper">
                <button className="mini-arrow mini-left" onClick={prevAltaSlide}>&#10094;</button>
                <img src={miniCarouselData[altaIndex].img} alt={miniCarouselData[altaIndex].caption} key={altaIndex} />
                <button className="mini-arrow mini-right" onClick={nextAltaSlide}>&#10095;</button>
              </div>
              <p className="mini-caption">{miniCarouselData[altaIndex].caption}</p>
              <div className="mini-dots">
                {miniCarouselData.map((_, idx) => (
                  <div key={idx} className={`mini-dot ${altaIndex === idx ? "active" : ""}`} onClick={() => setAltaIndex(idx)} />
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="articles-grid">
            {categoriasDestacadas.map((cat) => {
              // 1. Obtenemos el timestamp de la noticia más reciente de esta categoría (si la hay)
              const timestampNoticiaNueva = nuevasPorCategoria[cat.title.toLowerCase()];
              
              // 2. Buscamos en localStorage cuándo fue la última vez que el usuario entró a esta categoría
              const ultimaVisita = localStorage.getItem(`visited_${cat.slug}`);
              
              // 3. Decidimos si mostrar la etiqueta
              let isNew = false;
              if (timestampNoticiaNueva) {
                // Si nunca visitó la página, o su última visita fue ANTES de que se publicara la noticia nueva
                if (!ultimaVisita || parseInt(ultimaVisita) < timestampNoticiaNueva) {
                  isNew = true;
                }
              }
              ...
              return (
                ...
                >
                  {isNew && <span className="badge-nuevo-articulo">Nueva noticia</span>}
                  
                  <h3 style={{ color: cat.color }}>{cat.title}</h3>
                  <div className="article-icon-wrapper">
                    <img src={cat.icon} alt={cat.title} className="icon-static" />
                    <img src={cat.iconAnimated} alt={`${cat.title} animado`} className="icon-animated" />
                  </div>
                  ...
                  {selectedNews.images.slice(0, 4).map((img, index) => {
                    ...
                    return (
                      <div key={index} className="mosaic-item" onClick={() => openLightbox(selectedNews.images, index)}>
                        ...
                        {isLastAndHidden && <div className="mosaic-overlay"><span>+{fotosRestantes}</span></div>}
                      </div>
                    );
                  })}
                </div>
